...
class DataIngestion:
    ...
    def export_collection_as_dataframe(self) -> pd.DataFrame:
        try:
            ...
            if df.empty:
                raise NetworkSecurityException("MongoDB returned empty data")

            if "_id" in df.columns:
                df.drop(columns=["_id"], inplace=True)

            df.replace("na", pd.NA, inplace=True)
            logger.info(f"Data shape from MongoDB: {df.shape}")
            return df
    ...
    def export_data_into_feature_store(self, dataframe: pd.DataFrame) -> str:
        try:
            os.makedirs(
                os.path.dirname(self.data_ingestion_config.feature_storea_file_path),
                exist_ok=True
            )
            dataframe.to_csv(
